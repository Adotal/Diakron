...
class ParticipantDetailsScreen extends StatefulWidget {
  const ParticipantDetailsScreen({super.key, required this.viewModel});

  final ParticipantDetailsViewModel viewModel;
...
class _ParticipantDetailsScreenState extends State<ParticipantDetailsScreen> {
  ...
  void initState() {
    super.initState();
    widget.viewModel.deleteParticipant.addListener(_onDelete);
    widget.viewModel.updateParticipant.addListener(_onUpdate);
    widget.viewModel.changeValidationStatus.addListener(_onChangedValidation);
  }

  @override
  void didUpdateWidget(covariant ParticipantDetailsScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    oldWidget.viewModel.deleteParticipant.removeListener(_onDelete);
    widget.viewModel.deleteParticipant.addListener(_onDelete);

    oldWidget.viewModel.updateParticipant.removeListener(_onUpdate);
    widget.viewModel.updateParticipant.addListener(_onUpdate);
  ...
  @override
  void dispose() {
    widget.viewModel.deleteParticipant.removeListener(_onDelete);
    widget.viewModel.updateParticipant.removeListener(_onUpdate);
    ...
    return CustomScreen(
      ...
      actions: [
          ...
          onPressed: () {
            setState(() {
              ...
              if (!widget.viewModel.isEditing) {
                ...
                if (participant != null) {
                  _usernameController.text = participant.userName ?? '';
                  _surnamesController.text = participant.surnames ?? '';
                  _phoneNumberController.text = participant.phoneNumber ?? '';
                  _createdAtController.text = participant.createdAt.toString();
                  _isActive = participant.isActive;
        ...
        IconButton(
          ...
          onPressed: () {
            if (widget.viewModel.participant?.id != null) {
              _showDeleteConfirm(widget.viewModel.participant!.id!);
            }
          },
        ),
      ],
              ...
              builder: (context, _) {
                ...
                if (widget.viewModel.load.error) {
                  return ErrorIndicator(
                    title: "Error cargando participante",
                    label: "Try again",
                ...
                if (participant == null) {
                  return Center(
                    child: Text(
                      "No se encontró el centro \n ${widget.viewModel.participantId}",
                    ),
                  );
                }
                ...
                return Stack(
                  children: [
                    ListView(
                      ...
                      children: [
                        ...
                        ListenableBuilder(
                          ...
                          builder: (context, _) {
                            if (widget.viewModel.participant!.isActive!) {
                              return ListTile(
                                title: Text('Activo'),
                                leading: Icon(
                                  Icons.circle,
                        ...
                        _buildDataRow(
                          ...
                          participant.createdAt != null
                              ? DateFormat(
                                  'dd/MM/yyyy HH:mm',
                                ).format(participant.createdAt!.toLocal())
                              : 'N/A',
                          Icons.person_outline,
                    ...
                    ListenableBuilder(
                      listenable: widget.viewModel,
                      builder: (context, _) {
                        if (widget.viewModel.updateParticipant.running) {
                            ...
                            children: [
                              ...
                              ElevatedButton(
                                ...
                                onPressed: () {
                                  CustomAlertDialog.show(
                                    context: context,
                                    title: 'Validar participante',
                                    content:
                                        '¿Seguro que quieres validar este participante?',
                                    actionText: 'Validar',
  ...
  void _showUpdateConfirm() {
    CustomAlertDialog.show(
      ...
      actionText: 'Guardar',
      onPressed: () {
        // UPDATE EDITParticipant
        widget.viewModel.editedParticipant = widget.viewModel.editedParticipant
            ?.copyWith(
        ...
        // EXEC UPDATE
        widget.viewModel.updateParticipant.execute();
        setState(() {
          widget.viewModel.toggleEdit();
  ...
  void _showDeleteConfirm(String id) {
    CustomAlertDialog.show(
      ...
      actionText: 'Eliminar',
      onPressed: () async {
        await widget.viewModel.deleteParticipant.execute();
      },
      actionButtonColor: Colors.red,
    );
  }
  ...
  void _onUpdate() {
    if (widget.viewModel.updateParticipant.completed) {
      widget.viewModel.updateParticipant.clearResult();

      CustomSnackBar.showSuccess(context, message: "Participante Actualizado");
    }

    if (widget.viewModel.updateParticipant.error) {
      widget.viewModel.updateParticipant.clearResult();
  ...
  void _onDelete() {
    if (widget.viewModel.deleteParticipant.completed) {
      widget.viewModel.deleteParticipant.clearResult();
      CustomSnackBar.showSuccess(context, message: "Participante eliminado");
      context.pop();
    }

    if (widget.viewModel.deleteParticipant.error) {
      widget.viewModel.deleteParticipant.clearResult();
      ...
      CustomSnackBar.showError(
        ...
        onRetry: () {
          widget.viewModel.deleteParticipant.execute();
        },
      );
    }
  }
